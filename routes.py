...
class GraphService:
	...
	def __init__(self, graph_path: Path):
		...
		self._id_to_name = {node: data.get("label", node) for node, data in self.graph.nodes(data=True)}
		# Build reverse lookup: lowercased name -> node_id for search
		self._name_to_id = {name.lower().strip(): node_id for node_id, name in self._id_to_name.items()}
		...
		self._id_to_logo = {str(team["id"]): team.get("logo", "") for team in teams_list}
	...
	def find_path(self, start_name: str, end_name: str) -> PathResult:
		...
		for u, v in zip(path_nodes, path_nodes[1:]):
			...
			label = edge_data.get("label") or f"{self._id_to_name[u]} def. {self._id_to_name[v]}"
			edges.append({
